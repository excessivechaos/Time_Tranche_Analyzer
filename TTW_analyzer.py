...
def get_day_of_week_sheets(
    ...
    # This dictionary maps each day of the week to a numerical value
    day_to_num = {
        "Monday": 1,
        "Tuesday": 2,
        "Wednesday": 3,
    ...
    days_sorted = sorted(df["Day of Week"].unique(), key=lambda day: day_to_num[day])
    ...
    for day in days_sorted:
        ...
        # Calculate PCR or pnl avg for each group
        if calc_type == "PCR":
            df_day_calced = df_day_grouped.apply(calculate_pcr)
        else:
            df_day_calced = df_day_grouped.apply(calculate_avg_pnl)

        # Unstack if it's a MultiIndex
        if isinstance(df_day_calced.index, pd.MultiIndex):
            df_day_calced = df_day_calced.unstack()

        # Calculate rolling averages
        df_day_short_avg = df_day_calced.rolling(short_avg_period, min_periods=1).mean()
        df_day_long_avg = df_day_calced.rolling(long_avg_period, min_periods=1).mean()

        # Calculate weighted average
        df_day_weighted_avg = (
            short_weight * df_day_short_avg + long_weight * df_day_long_avg
        )

        # Sort the data in descending order by date
        df_day_weighted_avg.sort_index(ascending=False, inplace=True)

        # Convert df_pcr_day_weighted_avg to a DataFrame if it's a Series
        if isinstance(df_day_weighted_avg, pd.Series):
            df_day_weighted_avg = df_day_weighted_avg.to_frame()

        if calc_type == "PCR":
            # round to 3 decimal place which will later be displayed from .xxx to xx.x%
            df_day_weighted_avg = df_day_weighted_avg.applymap(lambda x: round(x, 3))
        else:
            # round to 2 decimal place
            df_day_weighted_avg = df_day_weighted_avg.applymap(lambda x: round(x, 2))

        # Concatenate the output DataFrame and the weighted PCR DataFrame
        df_day_output = pd.concat([df_labels, df_day_weighted_avg], axis=1)
...
def analyze(
    file, short_avg_period, long_avg_period, short_weight, long_weight, top_x, calc_type
):
    # Load the CSV file
    try:
        df = pd.read_csv(file)
    ...
    if calc_type == "PCR":
        ...
        # Calculate weighted average
        df_pcr_weighted_avg = (
            short_weight * df_pcr_short_avg + long_weight * df_pcr_long_avg
        )
    ...
    if calc_type == "PnL":
        # Calculate AVg PnL for each group
        df_avg_pnl = df_grouped.apply(calculate_avg_pnl)

        # Unstack if it's a MultiIndex
        if isinstance(df_avg_pnl.index, pd.MultiIndex):
            df_avg_pnl = df_avg_pnl.unstack()

        # Calculate rolling averages
        df_pnl_short_avg = df_avg_pnl.rolling(short_avg_period, min_periods=1).mean()
        df_pnl_long_avg = df_avg_pnl.rolling(long_avg_period, min_periods=1).mean()

        # Calculate weighted average
        df_pnl_weighted_avg = (
            short_weight * df_pnl_short_avg + long_weight * df_pnl_long_avg
        )

        # Calculate 1-month average PCR
        df_pnl_1mo_avg = df_avg_pnl.rolling(1, min_periods=1).mean()

        # Sort the data in descending order by date
        df_pnl_weighted_avg.sort_index(ascending=False, inplace=True)
        df_pnl_1mo_avg.sort_index(ascending=False, inplace=True)

        # Convert df_pnl_weighted_avg to a DataFrame if it's a Series
        if isinstance(df_pnl_weighted_avg, pd.Series):
            df_pnl_weighted_avg = df_pnl_weighted_avg.to_frame()

        # round to 2 decimal place
        df_pnl_weighted_avg = df_pnl_weighted_avg.applymap(lambda x: round(x, 2))
        df_pnl_1mo_avg = df_pnl_1mo_avg.applymap(lambda x: round(x, 2))

        # Create a new DataFrame for output, adding date range as index
        df_output_labels = pd.DataFrame(index=df_pnl_weighted_avg.index)
        for i, (date, row) in enumerate(df_pnl_weighted_avg.iterrows()):
            current_month_end = date.to_timestamp() + pd.offsets.MonthEnd(1)
            previous_month_start = (
                current_month_end - pd.DateOffset(months=long_avg_period - 1)
            ).replace(day=1)
            if i == 0:
                date_range_label = f"{end_date} - {previous_month_start.date()}"
            elif i == len(df_pnl_weighted_avg) - 1:
                date_range_label = f"{current_month_end.date()} - {start_date}"
            else:
                date_range_label = (
                    f"{current_month_end.date()} - {previous_month_start.date()}"
        ...
        df_output_1mo_avg = pd.DataFrame(index=df_pnl_1mo_avg.index)
        for i, (date, row) in enumerate(df_pnl_1mo_avg.iterrows()):
            current_month_end = date.to_timestamp() + pd.offsets.MonthEnd(1)
            previous_month_start = current_month_end.replace(day=1)
            if i == 0:
                date_range_label = f"{end_date} - {previous_month_start.date()}"
            elif i == len(df_pnl_1mo_avg) - 1:
                date_range_label = f"{current_month_end.date()} - {start_date}"
            else:
                date_range_label = (
                    f"{current_month_end.date()} - {previous_month_start.date()}"
        ...
        # Concatenate the output DataFrame and the weighted PCR DataFrame
        df_output = pd.concat([df_output_labels, df_pnl_weighted_avg], axis=1)

        # Concatenate the output DataFrame and the 1mo PCR DataFrame
        df_output_1mo_avg = pd.concat([df_output_1mo_avg, df_pnl_1mo_avg], axis=1)
    ...
    filename = os.path.join(
        path,
        (
            f"{org_filename}-TWAvg({calc_type})_{short_avg_period}mo-{long_avg_period}mo_{start_date} -"
            f" {end_date}.xlsx"
        ),
    )
    ...
    with pd.ExcelWriter(filename, engine="xlsxwriter") as writer:
        ...
        df_output.to_excel(writer, sheet_name=f"TW_{calc_type}", index=False)
        df_output_1mo_avg.to_excel(writer, sheet_name=f"1mo Avg {calc_type}", index=False)
        ...
        worksheets.append(writer.sheets[f"TW_{calc_type}"])
        worksheets.append(writer.sheets[f"1mo Avg {calc_type}"])
...
def main():
    ...
    layout = [
        ...
        [
            ...
            sg.Button("Cancel"),
            sg.Combo(["PCR", "PnL"], "PCR", key="-CALC_TYPE-", pad=(50, 0)),
            sg.Text("", size=(5, 1)),
            sg.Text(version),
        ],
    ]

    window = sg.Window("TimeTrending Weighted Analyzer", layout, resizable=True)
